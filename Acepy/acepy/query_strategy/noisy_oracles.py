...
class QueryNoisyOraclesCEAL(interface.BaseNoisyOracleQuery):
    ...
    initial_labeled_indexs: {list, np.ndarray, IndexCollection}
    ...
    def __init__(self, X, y, initial_labeled_indexs):
        super(QueryNoisyOraclesCEAL, self).__init__(X, y)
        ytype = type_of_target(self.y)
        if ytype in ['multilabel-indicator', 'multilabel-sequences']:
            warnings.warn("This query strategy doesnot support multi-label.",
                          category=FunctionWarning)
        assert (isinstance(initial_labeled_indexs, collections.Iterable))
        self._ini_ind = np.asarray(initial_labeled_indexs)
    ...
    def select(self, label_index, unlabel_index, oracles, model=None, **kwargs):
        ...
        return self.select_by_prediction_mat(label_index, unlabel_index, oracles, pred_unlab,
                                             n_neighbors=kwargs.pop('n_neighbors', 10))

    def select_by_prediction_mat(self, label_index, unlabel_index, oracles, predict, **kwargs):
        """Query from oracles. Return the index of selected instance and oracle.
        ...
        selected_oracle: int or str
            ...
            If a Oracles object is given, the oracle name will be returned.
        """
        Q_table, oracle_ind_name_dict = self._calc_Q_table(label_index, unlabel_index, oracles, predict,
                                                           n_neighbors=kwargs.pop('n_neighbors', 10))
    ...
    def _calc_Q_table(self, label_index, unlabel_index, oracles, pred_unlab, n_neighbors=10):
        ...
        oracles_iterset = list(range(len(oracles))) if oracles_type == 'list' else oracles.names()
