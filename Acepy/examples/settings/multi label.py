...
def main_loop(acebox, round, strategy):
    ...
    while len(label_ind) <= 120:
        ...
        # save
        st = acebox.State(select_index=select_labs, performance=perf, cost=cost)
        saver.add_state(st)
...
for round in range(5):
    ...
    audi_result.append(main_loop(acebox, round, strategy=audi))
    quire_result.append(main_loop(acebox, round, strategy=quire))
    # mmc_result.append(main_loop(acebox, round, strategy=mmc))
    # adaptive_result.append(main_loop(acebox, round, strategy=adaptive))
    random_result.append(main_loop(acebox, round, strategy=random))
